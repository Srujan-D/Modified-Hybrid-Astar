...
std::vector<std::vector<float>> voronoi_path() {
	...
	if(voronoi_start_id == voronoi_goal_id) {
	...
	for (int i = 0; i < sub_goals.size() - 1; ++i)	{
		...
		voronoi_sub_goal.id = i;
		voronoi_sub_goal.type = visualization_msgs::Marker::ARROW;
		voronoi_sub_goal.scale.x = 0.05;
		voronoi_sub_goal.scale.y = 0.2;
		voronoi_sub_goal.color.r = 0.0;
		...
		pt.y = sub_goals[i][1];
		pt.z = 0.0;
		voronoi_sub_goal.points.push_back(pt);
		pt.x = sub_goals[i+1][0];
		pt.y = sub_goals[i+1][1];
		pt.z = 0.0;
		voronoi_sub_goal.points.push_back(pt);
		voronoi_sub_goals.markers.push_back(voronoi_sub_goal);
