...
hybrid_astar_planner::HybridAStarPlanner temp_obj; 
...
void callback_goal_pose(const geometry_msgs::PoseStamped::ConstPtr& pose) {
	...
	temp_obj.makePlan(start_pose, goal_pose, temp);	
}
...
bool callback_planner(hybrid_astar::GlobalPath::Request &req, hybrid_astar::GlobalPath::Response &res) {
	...
	if (counter_for_planner < 7 || (int)dist>4)	{
		...
		printf("\n replanning stopped as there's less than 4 units to goal pose \n");
	}
	res.plan = path;
	return true;
}
